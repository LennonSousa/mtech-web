...
const RoofTypesPage: NextPage = () => {
    ...
    useEffect(() => {
        ...
        handleSelectedMenu('estimates-roofs-types');
    ...
    function handleOnDragEnd(result: DropResult) {
        if (result.destination) {
            ...
            const updatedListDocs = produce(roofTypes, draft => {
                if (draft) {
            ...
            if (updatedListDocs) {
                setRoofTypes(updatedListDocs);
                saveOrder(updatedListDocs);
            }
        }
    }

    async function saveOrder(list: RoofType[]) {
        list.forEach(async (doc, index) => {
            try {
                await api.put(`roofs/types/${doc.id}`, {
                    name: doc.name,
                    order: index
                });
                ...
                console.log('error to save estimates roofs types order');
                console.log(err)
            }
        });
    }
    ...
    return (
        <>
            <NextSeo
                ...
                description="Tipos de telhado da plataforma de gerenciamento da Plataforma solar."
                openGraph={{
                    ...
                    images: [
                        {
                            ...
                            alt: 'Tipos de telhado | Plataforma solar',
                        },
                        { url: `${process.env.NEXT_PUBLIC_API_URL}/assets/images/logo.jpg` },
                    ],
                }}
                                                            ...
                                                            <Droppable droppableId="lines">
                                                                {provided => (
                                                                    ...
                                                                    >
                                                                        <ListGroup>
                                                                            {
                                                                                roofTypes && roofTypes.map((roofType, index) => {
                                                                                    return <Draggable key={roofType.id} draggableId={roofType.id} index={index}>
                                                                                        {(provided) => (
                                                                                            <div
                                                        ...
                                                        <Col>
                                                            <Row>
                                                                <Col className="text-center">
                                                                    <p style={{ color: 'var(--gray)' }}>Nenhum tipo de telhado registrado.</p>
                                                                </Col>
                                                            </Row>
                                ...
                                <Modal show={showModalNewType} onHide={handleCloseModalType}>
                                    ...
                                    <Formik
                                        ...
                                        onSubmit={async values => {
                                            ...
                                            try {
                                                if (roofTypes) {
                                                    await api.post('roofs/types', {
                                                        name: values.name,
                                                        order: roofTypes.length,
                                                    });
                                                ...
                                                console.log('error create roofs types.');
                                                console.log(err);
                                            }
